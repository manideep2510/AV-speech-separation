...
model = tasnet.model
model.load_weights('/data/models/tasnet_ResNetLSTMLip_Lips_crm_236kTrain_2secondsClips_RootL2Loss_epochs20_lr2e-4_0.1decay10epochs_exp1/weights-19-50.0726.hdf5')
...
for n in range(num_100s):
    ...
    for i, item in enumerate(val_folders_pred):
        ...
        for j, item in enumerate(items):
            ...
            shutil.copy2('/data/lrs2/mvlrs_v1/pretrain/'+item[-34:-15]+'/'+item[-14:-9]+'.mp4','/' '/data/pred_tasnet_6000/'+item[-88:-35])

            samples_ = np.load('/data/lrs2/train/'+item[-88:-9]+'_samples.npy')
            scipy.io.wavfile.write('/data/pred_tasnet_6000/'+item[-88:-9]+'_original.wav', 16000, samples_[:length])
        ...
        scipy.io.wavfile.write('/data/pred_tasnet_6000/'+item[-88:-35]+'/mixed.wav', 16000, add_samples)


    val_folders_pred = sorted(glob.glob('/data/pred_tasnet_6000/*'), key=numericalSort)

    for fold in val_folders_pred:
        wavs = sorted(glob.glob(fold + '/*_original.wav'), key=numericalSort)
        file_sum_name = '/data/pred_tasnet_6000/'+fold[-53:]+'/mixed.wav'
        su = audios_sum(wavs,file_sum_name, volume_reduction=0)

    npys = sorted(glob.glob('/data/pred_tasnet_6000/*/*.npy'),key=numericalSort)
    for i in npys:
        os.remove(i)
