...
pesq1 = np.loadtxt('/home/gpudata/avinash/metric_results/' + path + '/pesq_val.txt')

snrs1 = np.loadtxt('/home/gpudata/avinash/metric_results/' + path + '/snr_val.txt')

sdrs1 = np.loadtxt('/home/gpudata/avinash/metric_results/' + path + '/sdr_val.txt')


pesq_mix = np.loadtxt('/home/gpudata/avinash/metric_results/' + path + '/pesq_mix.txt')

snrs_mix = np.loadtxt('/home/gpudata/avinash/metric_results/' + path + '/snr_mix.txt')

sdrs_mix = np.loadtxt('/home/gpudata/avinash/metric_results/' + path + '/sdr_mix.txt')


path = 'tdavss_LSGAN_ZEROGP_PS2_1350Norm_3speakers'

pesq2 = np.loadtxt('/home/gpudata/avinash/metric_results/' + path + '/pesq_val.txt')

snrs2 = np.loadtxt('/home/gpudata/avinash/metric_results/' + path + '/snr_val.txt')

sdrs2 = np.loadtxt('/home/gpudata/avinash/metric_results/' + path + '/sdr_val.txt')
...
# Plot
plt.figure()
plt.scatter(snrs_mix, pesq1-pesq_mix, s=area, c='green', alpha=0.5)
plt.scatter(snrs_mix, pesq2-pesq_mix, s=area, c='red', alpha=0.5)
plt.title('PESQ Improvement Vs Input SNR')
plt.xlabel('Input SNR')
plt.ylabel('PESQ Improvement')
plt.savefig('pesq_improv_vs_snr.png')

print('Max diff:', sorted((pesq2-pesq1).tolist())[-30:])
...
for i in range(len(snrs_mix)):
    if pesq_mix[i]>=-1 and pesq_mix[i] <0:
        temp0.append(pesq1[i])
    elif pesq_mix[i]>=0 and pesq_mix[i] <1:
        temp1.append(pesq1[i])
    elif pesq_mix[i]>=1 and pesq_mix[i] <2:
        temp2.append(pesq1[i])
    elif pesq_mix[i] >= 2 and pesq_mix[i] < 3:
        temp3.append(pesq1[i])
    '''elif snrs_mix[i]>=3 and snrs_mix[i] <4:
        temp4.append(snrs1[i])
    elif snrs_mix[i]>=4 and snrs_mix[i] <5:
        temp5.append(snrs1[i])'''
...
for i in range(len(snrs_mix)):
    if pesq_mix[i]>=-1 and pesq_mix[i] <0:
        temp0.append(pesq2[i])
    elif pesq_mix[i]>=0 and pesq_mix[i] <1:
        temp1.append(pesq2[i])
    elif pesq_mix[i]>=1 and pesq_mix[i] <2:
        temp2.append(pesq2[i])
    elif pesq_mix[i]>=2 and pesq_mix[i] <3:
        temp3.append(pesq2[i])
    '''elif snrs_mix[i]>=3 and snrs_mix[i] <4:
        temp4.append(snrs2[i])
    elif snrs_mix[i]>=4 and snrs_mix[i] <5:
        temp5.append(snrs2[i])'''
...
for i in range(len(snrs_mix)):
    if snrs_mix[i]>=-5 and snrs_mix[i] <-4:
        ...
        temp4.append(pesq1[i])
    elif snrs_mix[i]>=0 and snrs_mix[i] <1:
        temp5.append(pesq1[i])
    elif snrs_mix[i]>=1 and snrs_mix[i] <2:
        temp6.append(pesq1[i])
    elif snrs_mix[i]>=2 and snrs_mix[i] <3:
        temp7.append(pesq1[i])
    elif snrs_mix[i]>=3 and snrs_mix[i] <4:
        temp8.append(pesq1[i])
    elif snrs_mix[i]>=4 and snrs_mix[i] <=5:
        temp9.append(pesq1[i])
...
for i in range(len(snrs_mix)):
    if snrs_mix[i]>=-5 and snrs_mix[i] <-4:
        ...
        temp4.append(pesq2[i])
    elif snrs_mix[i]>=0 and snrs_mix[i] <1:
        temp5.append(pesq2[i])
    elif snrs_mix[i]>=1 and snrs_mix[i] <2:
        temp6.append(pesq2[i])
    elif snrs_mix[i]>=2 and snrs_mix[i] <3:
        temp7.append(pesq2[i])
    elif snrs_mix[i]>=3 and snrs_mix[i] <4:
        temp8.append(pesq2[i])
    elif snrs_mix[i]>=4 and snrs_mix[i] <=5:
        temp9.append(pesq2[i])
