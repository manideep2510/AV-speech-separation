...
import sys
sys.path.append('/data/AV-speech-separation')
...
from LipNet.lipnet.lip import LipNet
...
class VideoModel():
    ...
    def FullModel(self, lipnet_pretrained,weights=''):
      #  import tensorflow as tf
...
#        ip_phase = Lambda(lambda x : tf.expand_dims(x[:,:,:,1], axis = -1),name="ip_phase")(ip)#; print("ip_phase ", ip_phase.shape)  #takes phase from stack[magnitude,phase]
        ...
        lipnet_model=LipNet(pretrained=lipnet_pretrained,weights_path=weights)
