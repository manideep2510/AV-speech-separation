...
def Conv_Block_Audio(inputs,dialation_rate=1,stride=1,filters=256,kernel_size=3):
...
class TasNet(object):
    ...
    def build(self):
        ...
        #audio_processing
        self.outa=Conv_Block_Audio(self.audio,dialation_rate=1)
        self.outa=Conv_Block_Audio(self.outa,dialation_rate=2)
        self.outa=Conv_Block_Audio(self.outa,dialation_rate=4)
        self.outa=Conv_Block_Audio(self.outa,dialation_rate=8)
        self.outa=Conv_Block_Audio(self.outa,dialation_rate=16)
        self.outa=Conv_Block_Audio(self.outa,dialation_rate=32)
        ...
        if self.attention == True:
            ...
            #self.lipout=UpSampling1D(size=8)(self.outv1)
            self.q=concatenate([self.outv, self.outa],axis=-1)
            self.q=Conv_Block_Audio(self.q,dialation_rate=1,filters=512)
            self.q=Conv_Block_Audio(self.q,dialation_rate=2,filters=512)
            self.q=Conv_Block_Audio(self.q,dialation_rate=4,filters=512)
            self.q=Conv_Block_Audio(self.q,dialation_rate=8,filters=512)
            self.q=Conv_Block_Audio(self.q,dialation_rate=16,filters=512)
            self.q=Conv1D(256,1,activation='relu')(self.q)
            ...
            self.attn_out, self.attn_states = self.attn_layer([self.outv, self.q], verbose=False)
            print('attn_out:', self.attn_out.shape)
            print('attn_states:', self.attn_states.shape)

            
            self.fusion=concatenate([self.attn_out, self.outa],axis=-1)  # B, 200, 512
