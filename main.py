...
import cv2
from losses import l2_loss, sparse_categorical_crossentropy_loss, cross_entropy_loss
from models.lipnet import LipNet
from models.cocktail_lipnet import VideoModel
from data_generators import DataGenerator_train
...
folders_list_val = folders_list[-2500:]
...
model.compile(optimizer = Adam(lr=lrate), loss = cross_entropy_loss, metrics=['accuracy'])
...
path = 'Lipnet+cocktail_1in_1out_400k-train_epochs20_lr1e-4_0.322decay5epochs'
...
filepath='/home/manideep/models/' +  path+ '/weights-{epoch:02d}-{val_loss:.4f}-{val_acc:.4f}.hdf5'
checkpoint_save_weights = ModelCheckpoint(filepath, monitor='val_loss', save_best_only=True, mode='max')
...
history = model.fit_generator(DataGenerator_train(folders_list_train, batch_size),
                steps_per_epoch = np.ceil((len(folders_list_train)*2)/float(batch_size)),
                epochs=epochs,
                validation_data=DataGenerator_train(folders_list_val, batch_size), 
                validation_steps = np.ceil((len(folders_list_val)*2)/float(batch_size)),
