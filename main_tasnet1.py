...
from models.tdavss import TasNet as TasNet
...
wandb.init(name='tdavss_Baseline_L1Loss_BestVidEmbed_2speakers', notes='Batch size = 8, TDAVSS exact baseline, 256 filters in Audio processing and encoder, Per example input norm, lr = 5e-4',
...
folders_list_val = np.loadtxt(
    '/data/lrs2_val_comb2_new.txt', dtype='object').tolist()
...
model.compile(optimizer=Adam(lr=lrate, beta_1=0.5), loss=l1loss, metrics=[snr_acc])
...
path = 'tdavss_Baseline_L1Loss_BestVidEmbed_2speakers'
