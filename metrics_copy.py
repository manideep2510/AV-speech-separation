...
class Metrics(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        val_predict = np.asarray(self.model.predict_generator(DataGenerator_test(val_folders_100, self.batch_size), steps = np.ceil((len(self.val_folders_100))/float(self.batch_size))))
...
#        val_targ = val_targ[:, :80000]
        ...
        for i in range(batch):
            y_true = val_targ[i]
            y_pred = samples_pred[i]
            sdr_ = metric_eval(target_samples = y_true, predicted_samples = y_pred)
            sdrs.append(sdr_)'''

        _val_sdr, _ = metric_eval(target_samples = val_targ, predicted_samples = samples_pred)
