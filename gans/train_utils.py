...
def train_step(inputs, target, generator, discriminator, generator_optimizer, discriminator_optimizer, 
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        ...
        noise_inp = tf.random.normal(shape=tf.shape(inputs[1]), stddev=tf.nn.relu(1-(epoch/20))) #tf.nn.relu(1-(epoch/10))
        ...
        if gan_type == 'LSGAN':
            ...
            gradient_pen = gradient_penalty(inputs, target, noise_inp, gen_output, discriminator, center=1)
            ...
            gradient_pen = gradient_penalty(inputs, target, noise_inp, gen_output, discriminator, center=0)
    ...
    for epoch in range(epochs):
        ...
        for n, (inputs, target) in enumerate(DataGenerator_train(folders_list_train, batch_size=batch_size, norm=1)):
            ...
            gen_total_loss, gen_gan_loss, loss_snr, disc_loss, disc_real_loss, disc_fake_loss, disc_real_output, disc_generated_output, gradient_penalty = train_step(inputs, target, generator, 
                                                                            ...
                                                                            discriminator_optimizer, epoch, lambda_value, 'LSGAN')
        ...
        # Save Weights
        generator.save_weights('/home/manideepkolla/models/' + save_path + '/generator-' + str(int(epoch)) + '-' + str(np.round(val_acc, 4)) + '.tf')
        discriminator.save_weights('/home/manideepkolla/models/' + save_path + '/discriminator-' + str(int(epoch)) + '-' + str(np.round(val_acc, 4)) + '.tf')
