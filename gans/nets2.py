...
# Tdavss
def TDAVSS(time_dimensions=500,frequency_bins=257,n_frames=125, 
...
def tdavss_model():
    model = tdavss = TDAVSS(time_dimensions=200, frequency_bins=257, n_frames=50,
...
def Discriminator_SpectNorm(time_dimensions=500,frequency_bins=257,n_frames=125, phaseshuffle_rad=0, 
    ...
    # Audio Processing WITH Phase Shuffle
    outa=Conv_Block_Audio_disc_SpectNorm(audio,dialation_rate=1,filters=256)
    outa = phaseshuffle(outa)
    outa=Conv_Block_Audio_disc_SpectNorm(outa,dialation_rate=2,filters=256)
    outa = phaseshuffle(outa)
    outa=Conv_Block_Audio_disc_SpectNorm(outa,dialation_rate=4,filters=256)
    outa = phaseshuffle(outa)
    outa=Conv_Block_Audio_disc_SpectNorm(outa,dialation_rate=8,filters=256)
    outa = phaseshuffle(outa)
    outa=Conv_Block_Audio_disc_SpectNorm(outa,dialation_rate=16,filters=256)
    outa = phaseshuffle(outa)
    outa=Conv_Block_Audio_disc_SpectNorm(outa,dialation_rate=32,filters=256)
    outa = phaseshuffle(outa)
    outa=Conv_Block_Audio_disc_SpectNorm(outa,dialation_rate=64,filters=256)
    outa = phaseshuffle(outa)
    outa=Conv_Block_Audio_disc_SpectNorm(outa,dialation_rate=128,filters=256)
    outa = phaseshuffle(outa)
