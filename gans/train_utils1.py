...
def media_logging(val_files, generator, batch_size):
    ...
    pred_audios = []
    for inputs, target in DataGenerator_val(val_files, batch_size=batch_size, norm=1950.0):
            pred = generator([inputs[0], inputs[1]], training=True)
            pred = pred.numpy()
            pred = pred.tolist()
...
def train_step(inputs, target, generator, discriminator, generator_optimizer, discriminator_optimizer, 
    ...
    def gradient_penalty(inputs, target, noise_inp, gen_output, discriminator, center=0):
        ...
        with tf.GradientTape() as tape:
            ...
            d_hat = discriminator([inputs[2], inputs[1], interpolates, noise_inp], training=True)
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        gen_output = generator([inputs[0], inputs[1]], training=True)
        ...
        disc_real_output = discriminator([inputs[2], inputs[1], target, noise_inp1], training=True)
        disc_generated_output = discriminator([inputs[2], inputs[1], gen_output, noise_inp2], training=True)
...
def fit(folders_list_train, folders_list_val, folders_list_test, train_vids, generator, discriminator, generator_optimizer, 
    ...
    for epoch in range(epochs):
        ...
        for n, (inputs, target) in enumerate(DataGenerator_train(folders_list_train, train_vids, batch_size=batch_size, norm=1950.0, epoch=epoch+epoch_start)):
            ...
            values=[('gen_total_loss',gen_total_loss), ('gen_gan_loss',gen_gan_loss), ('gen_SNR_loss',loss_snr), 
                    ('disc_loss',disc_loss), ('grad_pen',gradient_penalty)] #'grad_pen',gradient_penalty)
        ...
        np.savetxt('/data/results/' + save_path + '/disc_real_loss.txt', disc_real_loss_all)
        np.savetxt('/data/results/' + save_path + '/disc_fake_loss.txt', disc_fake_loss_all)
        np.savetxt('/data/results/' + save_path + '/grad_pen.txt', grad_pen_all)
        ...
        plt.savefig('/data/results/' + save_path + '/disc_losses.png')
        plt.close()

        snrs = []
        for inputs, target in DataGenerator_val(folders_list_val, batch_size=batch_size, norm=1950.0):
            snrs.append(snr_acc(target, generator([inputs[0], inputs[1]], training=True)))
        val_acc = np.mean(snrs)
        
        print('Val SNR:', val_acc)

        '''with open('/data/results/' + save_path + '/val_snr.txt', "a") as myfile:
            myfile.write(str(val_acc)+'\n')'''

        # Save Weights
        generator.save_weights('/data/models/' + save_path + '/generator-' + str(int(epoch+epoch_start)) + '-' + str(np.round(val_acc, 4)) + '.tf')
        discriminator.save_weights('/data/models/' + save_path + '/discriminator-' + str(int(epoch+epoch_start)) + '-' + str(np.round(val_acc, 4)) + '.tf')
        ...
        with open('/data/results/' + save_path + '/logs.txt', "a") as myfile:
            myfile.write('Epoch: ' + str(epoch+epoch_start+1) + ' - ' + 'gen_total_loss: ' + str(np.mean(gen_total_loss_list)) + ' - ' + 'gen_gan_loss: ' + 
        ...
        '''if epoch+epoch_start >= 4 and epoch+epoch_start <= 35:
            #snrs_val = np.loadtxt('/data/results/' + save_path + '/val_snr.txt')
        ...
        '''if (epoch+epoch_start+1)%2 == 0:
            snrs_test = []
            for inputs, target in DataGenerator_val(folders_list_test, batch_size=batch_size, norm=1950.0):
                snrs_test.append(snr_acc(target, generator(inputs, training=True)))
            test_acc = np.mean(snrs_test)
            wandb.log({'Test SNR':test_acc}, commit=False)
            print('Test SNR after' , str(epoch+epoch_start+1), 'epochs:', test_acc)
        
            with open('/data/results/' + save_path + '/logs.txt', "a") as myfile:
                myfile.write('Test SNR: ' + str(np.mean(test_acc)) + '\n')'''
