...
from dataloaders import DataGenerator_val_samples_attention, DataGenerator_sampling_samples_attention
...
wandb.init(name='tdavss_Luong_attention_norm', notes='Luong_attention, Normalize input with 1350, Predict time samples directly.Mish Activation Function, 2 Second clips. 0.35 lr decay if no Val_loss Dec for 3epochs. TasNet with Resnet LSTM Lipnet. Loss is Si-SNR', project="av-speech-seperation")
...
#folders_list_train=folders_list_train[0:24]
...
#folders_list_val=folders_list_val[0:8]
#random.seed(30)
...
path = 'tdavss_Luong_attention_Normalize_ResNetLSTMLip_236kTrain_2secondsClips_epochs20_lr1e-4_0.35decayNoValDec2epochs_exp1'
#path='test_luong'
...
history = model.fit_generator(DataGenerator_sampling_samples_attention(folders_list_train, folders_per_epoch, int(batch_size)),
